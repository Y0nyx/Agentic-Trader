...
def clean_financial_data(data: pd.DataFrame, symbol: str = "") -> pd.DataFrame:
    ...
    for col in price_columns:
        if col in cleaned_data.columns:
            ...
            if missing_count > 0:
                ...
                logger.info(f"Filled {missing_count} missing values in {col} for {symbol}")
    ...
    if 'Volume' in cleaned_data.columns:
        ...
        if volume_missing > 0:
            ...
            logger.info(f"Filled {volume_missing} missing Volume values with 0 for {symbol}")
    ...
    if final_length != original_length:
        logger.info(f"Data cleaning removed {original_length - final_length} rows for {symbol}")
...
def _validate_price_relationships(data: pd.DataFrame, symbol: str = "") -> pd.DataFrame:
    ...
    if all(col in validated_data.columns for col in price_columns):
        ...
        invalid_high = validated_data['High'] < validated_data[['Open', 'Low', 'Close']].max(axis=1)
        invalid_low = validated_data['Low'] > validated_data[['Open', 'High', 'Close']].min(axis=1)
        
        invalid_count = (invalid_high | invalid_low).sum()
        if invalid_count > 0:
            logger.warning(f"Found {invalid_count} rows with invalid OHLC relationships for {symbol}")
...
def _detect_and_flag_outliers(data: pd.DataFrame, symbol: str = "") -> pd.DataFrame:
    ...
    mad = np.median(np.abs(pct_change_clean - median_change))  # Median Absolute Deviation
