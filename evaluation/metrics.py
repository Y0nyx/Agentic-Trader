...
def evaluate_performance(
    performance_report: PerformanceReport, benchmark_data: Optional[pd.DataFrame] = None
) -> Dict[str, Any]:
    """
...
def _calculate_advanced_metrics(
    ...
    risk_free_rate = 0.02
    excess_return = (
        portfolio["Total_Value"].iloc[-1] / performance_report.initial_capital - 1
    ) * 252
    sortino_ratio = (
        (excess_return - risk_free_rate) / downside_volatility
    ...
    cvar_5 = (
        returns[returns <= var_5].mean() if len(returns[returns <= var_5]) > 0 else 0
    )
    ...
    if transactions:
        consecutive_wins, consecutive_losses = _calculate_consecutive_trades(
            transactions
        )
    else:
...
def _calculate_benchmark_comparison(
    performance_report: PerformanceReport, benchmark_data: pd.DataFrame
) -> Dict[str, Any]:
    ...
    try:
        ...
        benchmark_final_value = performance_report.initial_capital * (
            1 + benchmark_return
        )

        # Calculate benchmark metrics
        benchmark_returns = (
            benchmark_period["Close"].pct_change(fill_method=None).dropna()
        )
        benchmark_volatility = (
            benchmark_returns.std() * np.sqrt(252) if len(benchmark_returns) > 1 else 0
        )

        # Benchmark drawdown
        benchmark_cummax = benchmark_period["Close"].expanding().max()
        benchmark_drawdown = (
            (benchmark_period["Close"] - benchmark_cummax) / benchmark_cummax
        ).min()

        # Performance comparison
        strategy_return = (
            performance_report.final_value - performance_report.initial_capital
        ) / performance_report.initial_capital
...
def calculate_risk_metrics(performance_report: PerformanceReport) -> Dict[str, float]:
    ...
    max_drawdown = abs(drawdown_series.min())
    avg_drawdown = (
        abs(drawdown_series[drawdown_series < 0].mean())
