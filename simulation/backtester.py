...
class Backtester:
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized Backtester with capital={initial_capital}, commission={commission:.3f}, slippage={slippage:.3f}"
        )
    ...
    def _execute_trade(
        self, price: float, quantity: float, trade_type: str, date: pd.Timestamp
    ) -> bool:
        """
        ...
        if trade_type == "BUY":
            ...
            if total_cost > self.cash:
                logger.warning(
                    f"Insufficient cash for trade on {date}: need {total_cost:.2f}, have {self.cash:.2f}"
                )
                return False
    ...
    def run_backtest(
        ...
        if data.empty or signals.empty:
            ...
            return PerformanceReport(
                pd.DataFrame(), [], self.initial_capital, self.initial_capital
            )
        ...
        return PerformanceReport(
            portfolio_df, self.transactions, self.initial_capital, final_value
        )
