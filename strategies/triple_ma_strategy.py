...
class TripleMovingAverageStrategy:
    ...
    def __init__(
        ...
        if not (short_window < medium_window < long_window):
            raise ValueError("Windows must be in ascending order: short < medium < long")
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        bullish_crossover = (
            (short_ma > medium_ma) &  # Current: short above medium
            (short_ma_prev <= medium_ma_prev) &  # Previous: short below or equal medium
            bullish_alignment &  # Bullish trend alignment
            valid_mask &
            valid_mask.shift(1)
        )
        
        bearish_crossover = (
            (short_ma < medium_ma) &  # Current: short below medium
            (short_ma_prev >= medium_ma_prev) &  # Previous: short above or equal medium
            bearish_alignment &  # Bearish trend alignment
            valid_mask &
            valid_mask.shift(1)
        )
