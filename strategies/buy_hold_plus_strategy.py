...
class BuyHoldPlusStrategy:
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result["Rolling_max"] = prices.rolling(
            window=252, min_periods=1
        ).max()  # 1 year rolling max

        # Current drawdown from peak
        result["Current_drawdown"] = (prices - result["Rolling_max"]) / result[
            "Rolling_max"
        ]
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(len(result)):
            ...
            market_stress = (
                current_rsi < self.stress_rsi_threshold
                or current_drawdown  # Extreme oversold
                < -self.drawdown_threshold  # Large drawdown
            )
    ...
    def get_strategy_summary(self, signals: pd.DataFrame) -> Dict[str, Any]:
        ...
        stress_periods = signals.get(
            "Market_Stress", pd.Series(False, index=signals.index)
        ).sum()
