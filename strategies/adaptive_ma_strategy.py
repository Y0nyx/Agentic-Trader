...
class AdaptiveMovingAverageStrategy:
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result["AMA"] = adaptive_moving_average(
    ...
    def generate_signals(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        bullish_crossover = (
            (prices > ama)
            & (prices_prev <= ama_prev)  # Current: price above AMA
            & valid_mask  # Previous: price below or equal AMA
            & valid_mask.shift(1)
        )

        bearish_crossover = (
            (prices < ama)
            & (prices_prev >= ama_prev)  # Current: price below AMA
            & valid_mask  # Previous: price above or equal AMA
            & valid_mask.shift(1)
        )
        ...
        if self.use_rsi_filter and "RSI" in result.columns:
            ...
            bullish_crossover = (
                bullish_crossover & (rsi_values < self.rsi_overbought) & rsi_valid
            )
            bearish_crossover = (
                bearish_crossover & (rsi_values > self.rsi_oversold) & rsi_valid
            )
