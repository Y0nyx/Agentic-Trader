...
class MovingAverageCrossStrategy:
    ...
    def __init__(
        self, short_window: int = 20, long_window: int = 50, price_column: str = "Close"
    ):
        ...
        logger.info(
            f"Initialized MovingAverageCrossStrategy with short={short_window}, long={long_window}"
        )
    ...
    def calculate_moving_averages(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        result[f"MA_{self.short_window}"] = (
            result[self.price_column]
            .rolling(window=self.short_window, min_periods=self.short_window)
            .mean()
        )

        result[f"MA_{self.long_window}"] = (
            result[self.price_column]
