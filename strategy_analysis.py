...
def create_performance_comparison_chart(data: pd.DataFrame, strategies_data: Dict, 
    ...
    plt.plot(data.index, buy_hold_drawdown * 100, label="Buy & Hold", 
    ...
    for i, (name, strategy_info) in enumerate(strategies_data.items()):
        ...
        plt.plot(portfolio_history.index, drawdown * 100, 
                label=name, linewidth=1.5, color=colors[i % len(colors)])
    ...
    rolling_sharpe_bh = buy_hold_returns.rolling(252).mean() / buy_hold_returns.rolling(252).std() * np.sqrt(252)
    plt.plot(data.index[252:], rolling_sharpe_bh.iloc[252:], label="Buy & Hold", 
             linewidth=2, color='black')
    ...
    for i, (name, strategy_info) in enumerate(strategies_data.items()):
        ...
        plt.plot(portfolio_history.index[252:], rolling_sharpe.iloc[252:], 
                label=name, linewidth=1.5, color=colors[i % len(colors)])
    ...
    for i, (name, trades, ret) in enumerate(zip(strategy_names, trade_counts, returns)):
        plt.scatter(trades, ret, s=100, color=colors_extended[i % len(colors_extended)], 
...
def analyze_market_periods(data: pd.DataFrame):
    ...
    print(f"  High Volatility periods (>75th percentile): {high_vol_periods.sum()} ({high_vol_periods.sum()/total_periods:.1%})")
    print(f"  Low Volatility periods (<25th percentile): {low_vol_periods.sum()} ({low_vol_periods.sum()/total_periods:.1%})")
    ...
    print(f"\nDrawdown Analysis:")
    print(f"  Periods with >20% drawdown: {major_drawdown_periods.sum()} ({major_drawdown_periods.sum()/total_periods:.1%})")
...
def test_strategy_on_subperiods(data: pd.DataFrame, strategy_class, params: Dict, strategy_name: str):
    ...
    for period_name, start_date, end_date in periods:
        try:
            ...
            print(f"{period_name}: Strategy {result['strategy_return']:.1f}% vs Benchmark {result['benchmark_return']:.1f}% "
