...
def main():
    ...
    if not available_files:
        print("No CSV files found! Please ensure data files are in the data/ directory.")
        return
    ...
    comprehensive_report = reporter.generate_comprehensive_report(
        benchmark_data=df,  # Use original price data for benchmark
        include_charts=True
    )
    ...
    print(f"Average Profit per Trade: ${detailed_metrics.get('avg_profit_per_trade', 0):.2f}")
    ...
    print(f"Max Consecutive Losses: {detailed_metrics.get('max_consecutive_losses', 0)}")
    
    if 'alpha' in detailed_metrics:
        print(f"Alpha vs Buy-and-Hold: {detailed_metrics['alpha_pct']:.2f}%")
        outperformed = "Yes" if detailed_metrics.get('outperformed_benchmark', False) else "No"
    ...
    print(f"- Total Return: {((performance_results.final_value / performance_results.initial_capital) - 1) * 100:.2f}%")
