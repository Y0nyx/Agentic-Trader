...
def test_strategy_performance(strategy_class, params: Dict, data: pd.DataFrame, 
                            backtester: Backtester, strategy_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        results = {
            ...
            "outperformed_benchmark": detailed_metrics.get("outperformed_benchmark", False),
        }
...
def run_multi_objective_optimization(strategy_class, param_grid: Dict, data: pd.DataFrame,
    ...
    objectives = [
        ...
        OptimizationObjective("num_trades", maximize=False, weight=0.1),  # Minimize trades
        OptimizationObjective("max_drawdown", maximize=False, weight=0.1),  # Minimize drawdown
    ]
    ...
    try:
        ...
        if best_params:
            best_results = test_strategy_performance(
                strategy_class, best_params, data, backtester, f"{strategy_name}_Optimized"
            )
...
def main():
    ...
    strategy_configs = [
        ...
        # Triple Moving Average
        {
            ...
            "name": "Triple_MA_SMA",
            "params": {"short_window": 5, "medium_window": 15, "long_window": 30, "ma_type": "sma"},
            "param_grid": {
                "short_window": [5, 8, 10],
        ...
        # Adaptive Moving Average
        {
            ...
            "name": "Adaptive_MA",
            "params": {"fast_period": 2, "slow_period": 30, "use_rsi_filter": True, "use_trend_filter": True},
            "param_grid": {
                "fast_period": [2, 3, 4],
        ...
        # Advanced MA with all filters
        {
            ...
            "params": {
                "short_window": 8, "long_window": 25, "ma_type": "ema",
                "use_volume_filter": True, "use_rsi_filter": True,
                "use_macd_filter": True, "use_bollinger_filter": True,
                "use_trend_filter": True, "adx_threshold": 25
            },
            "param_grid": {
    ...
    print("-" * 100)
    print(f"{'Strategy':<25} {'Return %':<10} {'Alpha %':<10} {'Sharpe':<8} {'Win Rate':<10} {'Trades':<8} {'Beat Benchmark':<15}")
    print("-" * 100)
    ...
    successful_strategies = [r for r in valid_results if r.get("outperformed_benchmark", False)]
    ...
    if successful_strategies:
        ...
        criteria_met = (
            best_strategy['total_return_pct'] > 2100 and
            best_strategy['win_rate'] > 0.5 and
            best_strategy['num_trades'] < 40 and
            best_strategy['sharpe_ratio'] > 5.0 and
            best_strategy['max_drawdown_pct'] > -25
        )
