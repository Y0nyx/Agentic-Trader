...
def main():
    ...
    for i, params in enumerate(buy_hold_configs):
        result = test_strategy(BuyHoldPlusStrategy, params, df, f"BuyHoldPlus_v{i+1}")
        if result:
    ...
    for i, params in enumerate(trend_configs):
        result = test_strategy(
            TrendFollowingStrategy, params, df, f"TrendFollowing_v{i+1}"
        )
        if result:
            results.append(result)
    ...
    for i, params in enumerate(ma_configs):
        result = test_strategy(
            MovingAverageCrossStrategy, params, df, f"MA_Optimized_v{i+1}"
        )
        if result:
            results.append(result)
    ...
    if results:
        ...
        print(
            f"{'Strategy':<25} {'Return %':<10} {'Alpha %':<10} {'Trades':<8} {'Win Rate':<10} {'Beats BM':<10}"
        )
        print("-" * 85)
        ...
        if successful_strategies:
            print(
                f"🎉 SUCCESS! Found {len(successful_strategies)} strategies that beat the benchmark!"
            )
            ...
            print(
                f"  Return: {best['total_return_pct']:.2f}% vs Benchmark {best['benchmark_return_pct']:.2f}%"
            )
            ...
            criteria_checks = {
                "Beats benchmark": best["total_return_pct"]
                > best["benchmark_return_pct"],
