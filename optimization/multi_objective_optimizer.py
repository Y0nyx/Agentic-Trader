...
class MultiObjectiveOptimizer:
    ...
    def _evaluate_objectives(
        self, performance_report, benchmark_data: Optional[pd.DataFrame] = None
    ) -> Dict[str, float]:
        """
        ...
        for obj in self.objectives:
            if obj.name in summary:
                ...
                value = summary.get(
                    "num_trades", 0
                )  # Don't negate here, handle in constraints
    ...
    def _check_constraints(self, objective_values: Dict[str, float]) -> bool:
        ...
        for constraint in self.constraints:
            if constraint.name not in objective_values:
                logger.warning(
                    f"Constraint metric '{constraint.name}' not found in results"
                )
                continue
    ...
    def optimize(
        ...
        for i, params in enumerate(param_combinations):
            try:
                ...
                # Evaluate objectives
                objective_values = self._evaluate_objectives(
        ...
        logger.info(
            f"Optimization completed. {len(valid_results)} valid results found."
        )

        if best_params is None:
            raise ValueError(
                "No valid parameter combinations found that satisfy constraints"
            )
        ...
        for i, result_i in enumerate(self.results):
            ...
            for j, result_j in enumerate(self.results):
                ...
                for obj in self.objectives:
                    ...
                    if (
                        obj_name not in result_i["objectives"]
                        or obj_name not in result_j["objectives"]
                    ):
                        continue
