...
class GridSearchOptimizer:
    ...
    def __init__(
        ...
        logger.info(f"Initialized GridSearchOptimizer with {self._count_combinations()} parameter combinations")
    ...
    def _validate_param_grid(self):
        ...
        for param_name, values in self.param_grid.items():
            if not isinstance(values, (list, tuple, np.ndarray)):
                raise ValueError(f"Parameter '{param_name}' values must be a list, tuple, or array")
            if len(values) == 0:
    ...
    def analyze_parameter_sensitivity(self) -> Dict[str, Dict[str, float]]:
        ...
        for param in self.param_grid.keys():
            if param in df.columns:
                ...
                param_stats = df.groupby(param)["score"].agg([
...
class OptimizationReport:
    ...
    def print_summary(self) -> str:
        ...
        if impact:
            ...
            for param, corr in sorted(impact.items(), key=lambda x: abs(x[1]), reverse=True):
