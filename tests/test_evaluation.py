...
class TestEvaluationMetrics(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.portfolio_history = pd.DataFrame({"Total_Value": values}, index=dates)
        ...
        self.benchmark_data = pd.DataFrame(
            {"Close": np.linspace(100, 115, 30)}, index=dates
        )
    ...
    def test_evaluate_performance_empty_report(self):
        ...
        empty_report = PerformanceReport(pd.DataFrame(), [], 10000, 10000)
...
class TestPerformanceReporter(unittest.TestCase):
    ...
    def setUp(self):
        ...
        portfolio_history = pd.DataFrame({"Total_Value": values}, index=dates)
        ...
        # Create benchmark data
        self.benchmark_data = pd.DataFrame(
            {"Close": np.linspace(100, 105, 20)}, index=dates
        )
