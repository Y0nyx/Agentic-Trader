...
class TestAdvancedStrategies(unittest.TestCase):
    ...
    def test_triple_ma_strategy_invalid_params(self):
        ...
        with self.assertRaises(ValueError):
            TripleMovingAverageStrategy(short_window=30, medium_window=20, long_window=10)
    ...
    def test_advanced_ma_strategy_initialization(self):
        ...
        strategy = AdvancedMAStrategy(
            ...
            use_volume_filter=True, use_rsi_filter=True, use_macd_filter=True
        )
    ...
    def test_advanced_ma_strategy_signal_generation(self):
        ...
        strategy = AdvancedMAStrategy(
            short_window=5, long_window=15, ma_type='ema',
            use_volume_filter=True, use_rsi_filter=True, use_macd_filter=False,
            use_bollinger_filter=False, use_trend_filter=False
        )
        signals = strategy.generate_signals(self.sample_data)
    ...
    def test_trend_following_strategy(self):
        ...
        # Check required columns exist
        required_columns = ['Signal', 'Position', 'EMA_trend', 'EMA_confirm', 'Trend_Score']
        for col in required_columns:
            self.assertIn(col, signals.columns)
